/**
 * GET /api/cron/profitspace-status-sync
 *
 * Runs every 30 minutes (Vercel Cron / pg_cron).
 * Polls Profitspace GET /api/v2/leads for the last 14 days and syncs
 * the buyer's CRM call status (saleStatus) back to our leads table.
 *
...
type ProfitspaceLead = {
  leadRequestIDEncoded: string;
  saleStatus: string | null;
  customerID: string | null;   // email
  countryCode: string | null;
  signupDate: string | null;
  hasFTD: number;              // 1 = has FTD
};

type ProfitspaceLeadsResponse = {
  items: ProfitspaceLead[];
  total: { items: number };
};
...
export async function GET(request: NextRequest) {
  ...
  // Get Profitspace API key from any active/testing integration
  ...
  // Poll last 14 days — wider window so we catch status changes on older leads
  const now = new Date();
  ...
  const url = new URL("https://profi-api.com/api/v2/leads");
  url.searchParams.set("fromDate", fromDate);
  url.searchParams.set("toDate", toDate);
  url.searchParams.set("itemsPerPage", "1000");

  let leads: ProfitspaceLead[] = [];
  try {
    ...
    const json = await resp.json() as ProfitspaceLeadsResponse;
    leads = Array.isArray(json.items) ? json.items : [];
  } catch (err) {
    return NextResponse.json({ error: String(err) }, { status: 502 });
  }

  // Only process leads that have a status set and an encoded ID to match on
  ...
  console.log(
    `[profitspace-status-sync] fetched=${leads.length} with_status=${leadsWithStatus.length} synced=${synced} unchanged=${unchanged} not_found=${notFound}`
  );
