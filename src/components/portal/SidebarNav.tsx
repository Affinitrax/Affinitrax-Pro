...
function NavLink({ href, label, icon }: NavItem) {
  ...
  return (
    ...
    >
      <span className={isActive ? "text-[#00d4ff]" : ""}>{icon}</span>
      {label}
      {isActive && (
...
export default function SidebarNav({ isAdmin }: { isAdmin: boolean }) {
  return (
    <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      ...
      <NavLink href="/portal/integrations" label="Integrations" icon={ICONS.integrations} />
      <NavLink href="/portal/settings" label="Settings" icon={ICONS.settings} />

      {isAdmin && (
        <>
          <div className="pt-3 pb-1 px-3">
            <p className="text-[#334155] text-[10px] font-semibold uppercase tracking-widest">Admin</p>
          </div>
          <NavLink href="/portal/admin/users" label="Partners" icon={ICONS.partners} />
          <NavLink href="/portal/admin/enroll" label="Enroll Partner" icon={ICONS.enroll} />
